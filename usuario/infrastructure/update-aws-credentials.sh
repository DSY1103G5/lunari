...
if [ ! -f "$HOME/.aws/credentials" ]; then
    ...
    echo -e "${YELLOW}Get new credentials from AWS Academy and run:${NC}"
    echo "  mkdir -p ~/.aws"
    echo "  cat > ~/.aws/credentials << 'CREDS'"
    echo "  [default]"
...
ssh -o ConnectTimeout=10 -i "$SSH_KEY" "$EC2_HOST" "pkill -f 'java -jar' || true"
sleep 2
ssh -o ConnectTimeout=10 -i "$SSH_KEY" "$EC2_HOST" 'bash -c "nohup java -jar app.jar --spring.profiles.active=prod > app.log 2>&1 < /dev/null &"'
sleep 1
