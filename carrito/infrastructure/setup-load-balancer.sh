...
# LUNARi Carrito Service - Application Load Balancer Setup Script
# Usage: ./setup-load-balancer.sh [project-id] [region] [service-name] [domain] [cert-file] [key-file]
# Example with Google-managed SSL: ./setup-load-balancer.sh lunari-microservices us-central1 carrito-service cart.aframuz.dev
# Example with custom SSL: ./setup-load-balancer.sh lunari-microservices us-central1 carrito-service cart.aframuz.dev /path/to/cert.pem /path/to/key.pem
...
# Header
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  LUNARi Carrito - Load Balancer Setup${NC}"
echo -e "${GREEN}========================================${NC}\n"
...
if gcloud compute addresses describe lunari-carrito-ip --global &>/dev/null; then
    log_info "Static IP already exists"
else
    gcloud compute addresses create lunari-carrito-ip \
      --global \
      --ip-version IPV4
    log_success "Static IP created"
fi

STATIC_IP=$(gcloud compute addresses describe lunari-carrito-ip \
  --global \
...
echo ""
echo "  Type: A"
echo "  Name: cart  (subdomain part)"
echo "  Value: $STATIC_IP"
echo "  TTL: 3600 (or Auto)"
echo ""
echo "Example for common providers:"
echo "  - Cloudflare: DNS → Add record → Type: A → Name: cart → IPv4: $STATIC_IP"
echo "  - GoDaddy: DNS Management → Add → Type: A → Host: cart → Points to: $STATIC_IP"
echo "  - Namecheap: Advanced DNS → Add New Record → Type: A Record → Host: cart → Value: $STATIC_IP"
echo ""
...
if gcloud compute network-endpoint-groups describe carrito-neg --region=$REGION &>/dev/null; then
...
if gcloud compute backend-services describe carrito-backend --global &>/dev/null; then
    log_info "Backend service already exists"
else
    gcloud compute backend-services create carrito-backend \
      --global \
...
if gcloud compute url-maps describe carrito-lb --global &>/dev/null; then
    log_info "URL map already exists"
else
    gcloud compute url-maps create carrito-lb \
      --default-service=carrito-backend
...
if [ "$USE_CUSTOM_CERT" = true ]; then
    ...
    if gcloud compute ssl-certificates describe carrito-cert --global &>/dev/null; then
        log_info "SSL certificate already exists"
        CERT_TYPE=$(gcloud compute ssl-certificates describe carrito-cert \
          --global \
          --format="value(type)")
        log_info "Certificate type: $CERT_TYPE"
    else
        gcloud compute ssl-certificates create carrito-cert \
    ...
    if gcloud compute ssl-certificates describe carrito-cert --global &>/dev/null; then
        log_info "SSL certificate already exists"
        CERT_STATUS=$(gcloud compute ssl-certificates describe carrito-cert \
          --global \
...
if gcloud compute target-https-proxies describe carrito-https-proxy --global &>/dev/null; then
    log_info "HTTPS proxy already exists"
else
    gcloud compute target-https-proxies create carrito-https-proxy \
      --ssl-certificates=carrito-cert \
      --url-map=carrito-lb
...
if gcloud compute url-maps describe carrito-lb-http --global &>/dev/null; then
    log_info "HTTP URL map already exists"
else
    gcloud compute url-maps create carrito-lb-http \
      --default-service=carrito-backend
    log_success "HTTP URL map created"
fi

if gcloud compute target-http-proxies describe carrito-http-proxy --global &>/dev/null; then
    log_info "HTTP proxy already exists"
else
    gcloud compute target-http-proxies create carrito-http-proxy \
      --url-map=carrito-lb-http
...
if gcloud compute forwarding-rules describe carrito-https-forwarding-rule --global &>/dev/null; then
    log_info "HTTPS forwarding rule already exists"
else
    gcloud compute forwarding-rules create carrito-https-forwarding-rule \
      --global \
      --target-https-proxy=carrito-https-proxy \
      --address=lunari-carrito-ip \
      --ports=443
    log_success "HTTPS forwarding rule created"
fi

log_info "Creating HTTP forwarding rule..."
if gcloud compute forwarding-rules describe carrito-http-forwarding-rule --global &>/dev/null; then
    log_info "HTTP forwarding rule already exists"
else
    gcloud compute forwarding-rules create carrito-http-forwarding-rule \
      --global \
      --target-http-proxy=carrito-http-proxy \
      --address=lunari-carrito-ip \
      --ports=80
...
if [ "$CERT_STATUS" = "ACTIVE" ]; then
    ...
    echo ""
    echo "  API Endpoint:"
    echo "    curl https://$DOMAIN/api/v1/carts/{userId}"
    echo ""
    echo "  Swagger UI:"
    ...
    echo "    gcloud compute ssl-certificates describe carrito-cert --global"
    echo ""
...
if [ "$USE_CUSTOM_CERT" = false ] && [ "$CERT_STATUS" != "ACTIVE" ]; then
    ...
    echo "  watch -n 10 'gcloud compute ssl-certificates describe carrito-cert --global --format=\"value(managed.status)\"'"
    echo ""
fi
...
echo "     gcloud compute backend-services update carrito-backend --global --enable-cdn"
echo ""
