...
# LUNARi Inventario Service - Application Load Balancer Setup Script
# Usage: ./setup-load-balancer.sh [project-id] [region] [service-name] [domain] [cert-file] [key-file]
# Example with Google-managed SSL: ./setup-load-balancer.sh lunari-microservices us-central1 inventario-service inventory.aframuz.dev
# Example with custom SSL: ./setup-load-balancer.sh lunari-microservices us-central1 inventario-service inventory.aframuz.dev /path/to/cert.pem /path/to/key.pem
...
DOMAIN="${4:-inventory.aframuz.dev}"
CERT_FILE="${5}"
KEY_FILE="${6}"
...
# Header
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  LUNARi Inventario - Load Balancer Setup${NC}"
echo -e "${GREEN}========================================${NC}\n"
...
if gcloud compute addresses describe lunari-inventario-ip --global &>/dev/null; then
    log_info "Static IP already exists"
else
    gcloud compute addresses create lunari-inventario-ip \
      --global \
      --ip-version IPV4
    log_success "Static IP created"
fi

STATIC_IP=$(gcloud compute addresses describe lunari-inventario-ip \
  --global \
...
echo ""
echo "  Type: A"
echo "  Name: inventory  (subdomain part)"
echo "  Value: $STATIC_IP"
echo "  TTL: 3600 (or Auto)"
echo ""
echo "Example for common providers:"
echo "  - Cloudflare: DNS → Add record → Type: A → Name: inventory → IPv4: $STATIC_IP"
echo "  - GoDaddy: DNS Management → Add → Type: A → Host: inventory → Points to: $STATIC_IP"
echo "  - Namecheap: Advanced DNS → Add New Record → Type: A Record → Host: inventory → Value: $STATIC_IP"
echo ""
...
if gcloud compute network-endpoint-groups describe inventario-neg --region=$REGION &>/dev/null; then
...
if gcloud compute backend-services describe inventario-backend --global &>/dev/null; then
    log_info "Backend service already exists"
else
    gcloud compute backend-services create inventario-backend \
      --global \
...
if gcloud compute url-maps describe inventario-lb --global &>/dev/null; then
    log_info "URL map already exists"
else
    gcloud compute url-maps create inventario-lb \
      --default-service=inventario-backend
...
if [ "$USE_CUSTOM_CERT" = true ]; then
    ...
    if gcloud compute ssl-certificates describe inventario-cert --global &>/dev/null; then
        log_info "SSL certificate already exists"
        CERT_TYPE=$(gcloud compute ssl-certificates describe inventario-cert \
          --global \
          --format="value(type)")
        log_info "Certificate type: $CERT_TYPE"
    else
        gcloud compute ssl-certificates create inventario-cert \
    ...
    if gcloud compute ssl-certificates describe inventario-cert --global &>/dev/null; then
        log_info "SSL certificate already exists"
        CERT_STATUS=$(gcloud compute ssl-certificates describe inventario-cert \
          --global \
...
if gcloud compute target-https-proxies describe inventario-https-proxy --global &>/dev/null; then
    log_info "HTTPS proxy already exists"
else
    gcloud compute target-https-proxies create inventario-https-proxy \
      --ssl-certificates=inventario-cert \
      --url-map=inventario-lb
...
if gcloud compute url-maps describe inventario-lb-http --global &>/dev/null; then
    log_info "HTTP URL map already exists"
else
    gcloud compute url-maps create inventario-lb-http \
      --default-service=inventario-backend
    log_success "HTTP URL map created"
fi

if gcloud compute target-http-proxies describe inventario-http-proxy --global &>/dev/null; then
    log_info "HTTP proxy already exists"
else
    gcloud compute target-http-proxies create inventario-http-proxy \
      --url-map=inventario-lb-http
...
if gcloud compute forwarding-rules describe inventario-https-forwarding-rule --global &>/dev/null; then
    log_info "HTTPS forwarding rule already exists"
else
    gcloud compute forwarding-rules create inventario-https-forwarding-rule \
      --global \
      --target-https-proxy=inventario-https-proxy \
      --address=lunari-inventario-ip \
      --ports=443
    log_success "HTTPS forwarding rule created"
fi

log_info "Creating HTTP forwarding rule..."
if gcloud compute forwarding-rules describe inventario-http-forwarding-rule --global &>/dev/null; then
    log_info "HTTP forwarding rule already exists"
else
    gcloud compute forwarding-rules create inventario-http-forwarding-rule \
      --global \
      --target-http-proxy=inventario-http-proxy \
      --address=lunari-inventario-ip \
      --ports=80
...
if [ "$CERT_STATUS" = "ACTIVE" ]; then
    ...
    echo ""
    echo "  API Endpoint:"
    echo "    curl https://$DOMAIN/api/v1/services"
    echo ""
    echo "  Swagger UI:"
    ...
    echo "    gcloud compute ssl-certificates describe inventario-cert --global"
    echo ""
...
if [ "$USE_CUSTOM_CERT" = false ] && [ "$CERT_STATUS" != "ACTIVE" ]; then
    ...
    echo "  watch -n 10 'gcloud compute ssl-certificates describe inventario-cert --global --format=\"value(managed.status)\"'"
    echo ""
fi
...
echo "     gcloud compute backend-services update inventario-backend --global --enable-cdn"
echo ""
echo "  2. Enable Cloud Armor (DDoS protection):"
echo "     See GCP documentation for Cloud Armor setup"
echo ""
